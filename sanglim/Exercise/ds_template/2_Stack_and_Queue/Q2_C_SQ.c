...
void removeEvenValues(Stack *s)
{
	...
	temp_array = (int *)malloc(stack_size * sizeof(int));

	int i = 0;				 // temp_array에 저장할 위치를 추적하는 인덱스
	while (!isEmptyStack(s)) // 스택 s가 비어있지 않는 동안 반복
	{
		temp_item = pop(s);		   // 스택에서 요소를 꺼냄
		temp_array[i] = temp_item; // 꺼낸 요소를 temp_array에 저장
		i++;					   // 인덱스 증가
	}

	for (--i; i >= 0; i--)
	{
		if (temp_array[i] % 2) // 요소가 홀수면
		{
			push(s, temp_array[i]); // 다시 스택s로 넣어줌
		}
	}

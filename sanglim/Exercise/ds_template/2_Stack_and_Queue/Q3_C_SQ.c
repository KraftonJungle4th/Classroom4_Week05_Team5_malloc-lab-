...
int isStackPairwiseConsecutive(Stack *s)
{
	...
	// 스택이 비어있는 경우 not pairwise consecutive notPC
	if (s->ll.size == 0 || s->ll.size % 2)
		return 0;

	ListNode *temp = s->ll.head; //
	short isAscOrDesc = 0;
	isAscOrDesc = temp->next->item - temp->item;

	if (isAscOrDesc == 0)
		return 0;
	else if (isAscOrDesc == 1 || isAscOrDesc == -1)
	{
	}
	else
		return 0;
	...
	{
		if (temp->item + isAscOrDesc != temp->next->item)
			return 0;

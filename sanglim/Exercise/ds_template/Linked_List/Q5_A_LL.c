...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	...
	int count = 0;
	// 원본 리스트의 앞쪽 반을 결과 리스트에 추가
	while (count < half + total_size % 2)
	{
		// 결과 리스트에 노드 추가 (값 복사)
		insertNode(resultFrontList, count, cur->item);
	...
	prev->next = NULL; // 결과 리스트의 끝을 나타내는 노드의 next를 NULL로 설정
	// cur = ll->head;	   // 나머지 원본 리스트의 노드를 결과 리스트에 추가

	count = 0;
	while (count < half)
	{
		insertNode(resultBackList, count, cur->item); // 결과 리스트에 노드 추가 (값 복사)
		resultBackList->size++;						  // 결과 리스트의 크기 증가

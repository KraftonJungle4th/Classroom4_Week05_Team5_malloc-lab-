...
// 전방 선언해줌
static void *heap_listp = NULL; // 할당기
...
static void *extend_heap(size_t words)
{
    ...
    PUT(HDRP(NEXT_BLKP(bp)), PACK(0, 1)); // end header 삽입,나중에 만들어지는 블록 헤더 여기에 넣음

    return coalesce(bp); // 주위 가용블록 있는지 확인및 병합후 반환
}
...
static void *coalesce(void *bp) // 주변에 합칠 것 있음 합치기
{
...
static void *find_fit(size_t asize)
{
    ...
    while (GET_SIZE(HDRP(bp)) > 0) // 해당 블록 사이즈가 0보다 크다면
    {
        ...
        {                   // 가용 상태이고, 사이즈가 적합하면
            last_bp = bp;   // 저장할 block 포인터 전역 변수 저장(마지막 할당된 block)!!!!!!!!!!!!
            return bp;      // alloc 해야할 block
        }                   // 해당 블록 포인터 리턴
...
static void *next_fit(size_t asize)
{
    ...
    // {
    //     return find_fit(asize); // find_fit 실행
    // }

    void *bp = NEXT_BLKP(last_bp); // find_fit 이후 block point
    // void *bp = last_bp;
    ...
    {                   // 해당 블록 사이즈 0보다 크고
        if (!GET_ALLOC(HDRP(bp)) && (asize <= GET_SIZE(HDRP(bp))))
            return bp;
                     // free, 가용 블록이 찾는 사이즈 보다 클 때(외부 단편화 No)
                     // last_bp = bp;
                     // 할당할 블록 return
        bp = NEXT_BLKP(bp);
    }
